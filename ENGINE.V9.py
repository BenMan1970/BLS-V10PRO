...
import io
import json
import re  # ← AJOUT : nécessaire pour _fix_pdf_css
...
# ═══════════════════════════════════════════════════════════════════════════
# AJOUT : Correctif chirurgical CSS pour le rendu PDF
# ═══════════════════════════════════════════════════════════════════════════
def _fix_pdf_css(html: str) -> str:
    """
    Correctif chirurgical du CSS de rendu PDF — aucune régression métier.

    Fix #1 — Marges blanches vides de 22mm sur les pages 2+ :
      Remplace le @page unique par deux règles segmentées :
        • @page :first → marge haute 22mm + @top-left (en-tête pageheader)
        • @page        → marge haute 8mm uniquement (pas d'en-tête réservé)

    Fix #2 — Duplication du bandeau de contexte :
      Injecte @media print { .print-ctx-bar { display: none !important; } }
      en fin de <style>. Le bandeau .page-subbar (dans .page-running) reste
      l'unique source d'affichage, éliminant le doublon.

    Sécurité :
      - Si @page ne contient pas '22mm' → aucun remplacement (pas de régression)
      - Si </style> est introuvable → aucune injection
      - Les règles injectées sont suffixées → spécificité CSS les rend prioritaires
    """
    # ──────────────────────────────────────────────────────────
    # FIX 1 : Segmenter @page en @page :first + @page
    # ──────────────────────────────────────────────────────────
    page_match = re.search(r'@page\s*\{', html)
    if page_match:
        # Vérifier que ce bloc contient bien la marge 22mm ciblée
        scan = html[page_match.start():page_match.start() + 600]
        if '22mm' in scan:
            start = page_match.start()
            # Trouver l'accolade fermante correspondante (gestion imbriquée)
            depth = 0
            end = None
            for i in range(page_match.end() - 1, len(html)):
                if html[i] == '{':
                    depth += 1
                elif html[i] == '}':
                    depth -= 1
            ...
            if end is not None:
                ...
                html = html[:start] + new_page_css + html[end:]

    # ──────────────────────────────────────────────────────────
    # FIX 2 : Masquer .print-ctx-bar en impression
    # ──────────────────────────────────────────────────────────
    # Injection en fin de </style> : la dernière règle !important l'emporte.
    # Ne touche PAS aux règles existantes — surcharge CSS pure.
    style_end = html.rfind('</style>')
    if style_end != -1:
        inject = "\n@media print { .print-ctx-bar { display: none !important; } }\n"
        html = html[:style_end] + inject + html[style_end:]
    ...
    with st.spinner("Pipeline en cours..."):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                # ═══════════════════════════════════════════════════════
                # AJOUT : Correctif PDF — marges blanches + duplication
                # ═══════════════════════════════════════════════════════
                html = _fix_pdf_css(html)
                # ═══════════════════════════════════════════════════════
